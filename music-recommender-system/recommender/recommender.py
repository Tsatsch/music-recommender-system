...
MOOD = {0: "Unsure", 1: "Happy", 2: "Romantic", 3: "Sad", 4: "Focused", 5: "Energetic"}
...
def main():
    user_input_playlist = "Nishant2500.csv"

    # parse configs and extablish connections to
    # Spotfiy and the database
    config, secrets = parser.setup()
    sp = Spotify(config=config, secrets=secrets)
    db = Database(config=config, secrets=secrets)
    db_data = db.get_all_data("spotify")

    # load models
    rf_model = joblib.load("trained_models/RFmodel.pkl")
    km_model = joblib.load("trained_models/Kstd6.pkl")

    # preprocess data format and get the audio features, normalization and weights.
    # db_full is with genre and mood
    data_playlist_pre_processed, num_user_songs, db_full = preprocess_data_playlist(
        db_data, user_input_playlist, sp, rf_model, km_model
    )
    # recommend with the preprocessed playlist data
    given_songs, recommendations = get_recomendation_KNN(
        data_playlist_pre_processed, num_user_songs
    )
    ...
    # visualize results
    print(
        f"These are the songs recommendations for you. BTW, it is {weather} outiside!"
    )
    table = visualize(result_fltrd, sp)
...
def preprocess_playlist_1(user_file, sp_conn):
    ...
    for j in range(limitcount + 2):
        songleft = l - j * limit
        if songleft > limit:
            Features = sp_conn.audio_features(
...
def preprocess_playlist(db_data, playlist, sp_conn):
    ...
    playlist_uris = [
        x[0].split("spotify:track:")[1] for x in playlist
    ]  # todo THIS NEEDS SLICE
    ...
    for j in range(limitcount + 2):
        ...
        if songleft > limit:
            features = sp_conn.audio_features(
                uris_not_in_db[j * limit : j * limit + limit]
            )
            ...
            features = sp_conn.audio_features(
                uris_not_in_db[j * limit : j * limit + songleft]
            )
            for feat in features:
                result = sp_conn.track(feat["id"])
                song = [
...
def preprocess_data_playlist(
    raw_data, user_playlist_file, spotify_conn, rf_model, kn_model
):
    """@raw_data: list of songs with their features from db
    @user_playlist: csv with uris
    @return: pre-processed set of songs incl user_song"""
    ...
    weighted_norm_data = [
        tuple(weighted_entry) for weighted_entry in weighted_norm_data
    ]
...
def get_recomendation_KNN(cooked_data, num_of_user_songs):
    ...
    user_songs_neigbors = (
        []
    )  # first element of list = index of user_song (id of song itself), value = list of distances and list of neigbors for that song
    ...
    all_best_songs = (
        []
    )  # here is just a bunch of recommendation independent from original song/ removed users song
    user_songs = []
...
def visualize(data, spotify):
    recommendations_result = []
    for recomendated_data in data:
        res = spotify.track(recomendated_data[0])
        ...
        recommendations_result.append(
            [
                ...
                recomendated_data[-3],
                recomendated_data[-2],
                MOOD[recomendated_data[-1]],
            ]
        )

    df = pd.DataFrame(
        recommendations_result,
        columns=["URI", "Artist", "Song", "Popularity", "Genre", "Mood"],
    )
